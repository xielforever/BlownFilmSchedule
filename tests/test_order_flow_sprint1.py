...
class _FakeCursor:
    ...
    def execute(self, sql, params=None):
        ...
        if "from production_orders o" in normalized and "limit %s offset %s" in normalized:
            ...
            screening_action_actor_filter = None
            ...
            if "lower(trim(latest_action.assignee))=%s" in normalized:
                ...
                param_index += 1
            if "lower(trim(latest_action.actor))=%s" in normalized:
                screening_action_actor_filter = params[param_index]
                param_index += 1
            ...
            screening_action_actor_unassigned_filter = "latest_action.actor is null" in normalized
            rows = []
            for row in self.db.production_orders.values():
                ...
                if (
                    ...
                    continue
                if screening_action_actor_unassigned_filter and (latest_action.get("actor") or "").strip():
                    continue
                rows.append({
                    **row,
        ...
        if normalized.startswith("select id, order_id, screening_status, business_bucket, screening_code"):
            ...
            actor_filter = None
            if "lower(trim(actor))=%s" in normalized:
                actor_filter = params[param_index]
            actor_unassigned_filter = "actor is null" in normalized
            rows = [
                dict(row)
...
class TestOrderFlowSprint1Routes(unittest.TestCase):
    ...
    def test_screening_exception_actions_can_be_filtered_by_actor(self):
        db = _FakeDb()
        db.order_screening_action_audit.extend([
            {
                "id": 47,
                "order_id": "ORD-ACTION-ACTOR-FILTER",
                "screening_status": "blocked",
                "business_bucket": "blocked_machine_capability",
                "screening_code": "no_eligible_machine",
                "action_type": "request_data_fix",
                "handling_status": "in_progress",
                "reason_text": "排程员甲退回维护",
                "assignee": "order-admin",
                "actor": " planner-a ",
                "details": {},
                "created_at": datetime(2026, 5, 24, 8, 0, tzinfo=timezone.utc),
        ...
        result = orders_router.get_order_screening_actions(
            "ORD-ACTION-ACTOR-FILTER",
            actor="planner-a",
            db=db,
            _=SimpleNamespace(username="planner"),
        )

        self.assertEqual([item["id"] for item in result["items"]], [47])
        self.assertEqual(result["items"][0]["actor"], " planner-a ")

    def test_screening_exception_actions_can_be_filtered_by_unassigned_actor(self):
        db = _FakeDb()
        db.order_screening_action_audit.extend([
            {
                "id": 49,
                "order_id": "ORD-ACTION-ACTOR-UNASSIGNED",
                "screening_status": "blocked",
                "business_bucket": "blocked_machine_capability",
                "screening_code": "no_eligible_machine",
                "action_type": "request_data_fix",
                "handling_status": "in_progress",
                "reason_text": "鎺掔▼鍛樻湭璁板綍",
                "assignee": "order-admin",
                "actor": None,
                "details": {},
                "created_at": datetime(2026, 5, 24, 8, 0, tzinfo=timezone.utc),
        ...
        result = orders_router.get_order_screening_actions(
            "ORD-ACTION-ACTOR-UNASSIGNED",
            actor="unassigned",
            db=db,
            _=SimpleNamespace(username="planner"),
        )

        self.assertEqual([item["id"] for item in result["items"]], [49])
        self.assertIsNone(result["items"][0]["actor"])
    ...
    def test_list_orders_filters_by_latest_screening_action_actor(self):
        db = _FakeDb()
        db.products.add("Film-A")
        for order_id in ["ORD-ACTOR-A", "ORD-ACTOR-B"]:
            db.production_orders[order_id] = {
                "order_id": order_id,
                "customer_id": "STANDARD",
                "product_type": "Film-A",
        ...
        db.order_screening_action_audit.extend([
            ...
            {
                "id": 34,
                "order_id": "ORD-ACTOR-B",
                "screening_status": "blocked",
                "business_bucket": "blocked_material",
                "screening_code": "material_not_ready",
                "action_type": "confirm_material",
                "handling_status": "waiting_external",
                "reason_text": "排程员乙确认物料",
                "assignee": "material-admin",
                "actor": "planner-b",
                "details": {},
                "created_at": datetime(2026, 5, 24, 9, 0, tzinfo=timezone.utc),
        ...
        result = orders_router.list_orders(
            status="PENDING",
            screening_action_actor="planner-a",
            q=None,
            page=1,
            size=50,
            db=db,
        ...
        self.assertEqual(result["total"], 1)
        self.assertEqual([item["order_id"] for item in result["items"]], ["ORD-ACTOR-A"])
        self.assertEqual(result["items"][0]["screening"]["latest_action"]["actor"], " planner-a ")

    def test_list_orders_filters_unassigned_screening_action_actor(self):
        db = _FakeDb()
        db.products.add("Film-A")
        for order_id in ["ORD-NO-ACTOR", "ORD-ACTOR"]:
            db.production_orders[order_id] = {
                "order_id": order_id,
                "customer_id": "STANDARD",
                "product_type": "Film-A",
        ...
        result = orders_router.list_orders(
            status="PENDING",
            screening_status="blocked",
            screening_action_actor="unassigned",
            q=None,
            page=1,
            size=50,
            db=db,
        ...
        self.assertEqual(result["total"], 1)
        self.assertEqual([item["order_id"] for item in result["items"]], ["ORD-NO-ACTOR"])
    ...
    def test_list_orders_summarizes_latest_screening_action_actors(self):
        db = _FakeDb()
        db.products.add("Film-A")
        for order_id in ["ORD-NO-ACTOR", "ORD-PLANNER-A", "ORD-PLANNER-B"]:
            db.production_orders[order_id] = {
                "order_id": order_id,
                "customer_id": "STANDARD",
                "product_type": "Film-A",
