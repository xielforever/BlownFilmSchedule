  ...
  test('records restricted screening override from the order pool', async ({ page, request }) => {
    ...
    const created = await apiJson(request, 'post', '/api/orders', {
      data: {
        ...
        cleanroom_req: 'Class_100K',
        order_class: 'NORMAL',
        due_date: '2026-05-20T08:00:00',
        material_available_time: '2026-06-01T08:00:00',
        reason_code: 'E2E_SETUP',
        reason_text: 'E2E setup for screening override',
      },
    });
    expect(created.ok()).toBeTruthy();
    cleanupOrderIds.push(orderId);
    ...
    await page.getByTestId('workbench-search').fill(orderId);
    await expect(page.getByTestId(`workbench-pending-order-${testIdPart(orderId)}`)).toBeVisible();
    await expect(page.getByTestId(`workbench-screening-override-action-${testIdPart(orderId)}`)).toContainText('\u7533\u8bf7\u8c41\u514d');

    page.once('dialog', async dialog => {
      expect(dialog.type()).toBe('prompt');
      await dialog.accept('E2E override confirmed');
    });
    await page.getByTestId(`workbench-screening-override-action-${testIdPart(orderId)}`).click();
