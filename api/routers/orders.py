...
def list_orders(
    ...
    if screening_action_actor:
        normalized_action_actor = screening_action_actor.strip().lower()
        if normalized_action_actor == "unassigned":
            where_clauses.append("(latest_action.actor IS NULL OR TRIM(latest_action.actor)='')")
        else:
            where_clauses.append("LOWER(TRIM(latest_action.actor))=%s")
            params.append(normalized_action_actor)
    if q:
        like = f"%{q.strip()}%"
    ...
    screening_bucket_counts = {value: 0 for value in SCREENING_BUSINESS_BUCKETS}
    cur.execute(f"""
        SELECT COALESCE(osc.business_bucket, osc.result->>'business_bucket', 'unknown') AS business_bucket,
        ...
        {where}
        GROUP BY COALESCE(osc.business_bucket, osc.result->>'business_bucket', 'unknown')
    """, params)
    for row in cur.fetchall():
        bucket_key = row.get("business_bucket") or "unknown"
        screening_bucket_counts[bucket_key] = int(row.get("cnt") or 0)
    ...
    action_actor_counts = {}
    cur.execute(f"""
        SELECT COALESCE(TRIM(latest_action.actor), 'unassigned') AS actor,
               count(DISTINCT o.order_id) AS cnt
        FROM production_orders o
        LEFT JOIN customers c ON o.customer_id = c.customer_id
        LEFT JOIN scheduled_tasks t ON o.order_id = t.order_id
        ...
        {where}
        GROUP BY COALESCE(TRIM(latest_action.actor), 'unassigned')
    """, params)
    for row in cur.fetchall():
    ...
    return {
        ...
        "page": page,
        "size": size,
        "screening_bucket_counts": screening_bucket_counts,
